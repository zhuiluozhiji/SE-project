...
- `README.md` 只负责环境配置和团队基础使用说明，后续除环境问题外不作为进度记录文件。
- `AGENTS.md` 现在同时承担“开发规范 + 当前进度记录”两类职责。
- `progress/总进度.md` 已弃用，不再作为当前有效进度来源。
- 后续如果需要更新项目阶段、当前优先任务、已完成事项或阻塞项，统一更新本文件。
...
```text
团队统一使用 Docker Compose 管理 mysql、backend、frontend
不再使用本地 venv / 本地 pip install / 本地 uvicorn / 本地 npm run dev 作为团队标准流程
后端测试统一通过 docker compose exec backend pytest 执行
数据库初始化统一通过 docker compose 启动 mysql 容器自动执行 schema.sql 和 seed.sql
```

### 7.4 当前优先任务

1. 把活动、推荐、日程相关 service 从 mock 替换为真实数据库查询。
2. 跑通登录、活动列表、活动详情主链路。
3. 跑通课程导入、冲突检测、加入日程最小闭环。
4. 补齐接口测试、联调记录和相关文档同步。
