\chapter{I. 最新文献回顾}\label{i.-ux6700ux65b0ux6587ux732eux56deux987e}
...
\section{2.1 系统特点}\label{ux7cfbux7edfux7279ux70b9}
...
\endlastfoot
工厂方法 & 日程事件统一创建 \\
单件 & \texttt{settings}、数据库会话工厂、Axios 实例、Pinia store \\
适配器 & CSV / Excel / OCR 课表导入 \\
外观 & \texttt{frontend/src/api/*.js} 对页面提供统一 API \\
代理 & 管理员后台的鉴权与拦截 \\
观察者 & Pinia 登录态与页面联动 \\
策略 & 推荐打分模型 \\
状态 & 课表导入流程状态机 \\
...
\section{1. 创建型模式}\label{ux521bux5efaux578bux6a21ux5f0f}

\subsection{a. 工厂方法（Factory
...
\texttt{schedule\_event}
或统一的日历展示结构上，但字段填充规则、类型映射和颜色标识并不完全相同。

如果在 \texttt{schedule\_service} 中直接用大量 \texttt{if/else}
拼接不同类型事件，逻辑会快速膨胀。因此，更合理的做法是为不同事件来源提供各自的创建器，例如
\texttt{CourseEventCreator}、\texttt{ActivityEventCreator} 和
\texttt{PersonalEventCreator}，再由 service
根据类型选择具体创建器。这样一来，活动加入日程、课表导入和手工新增都能共享统一创建入口，而不必在同一个函数里硬编码所有构造细节。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-1 工厂方法模式：统一创建不同来源的日程事件}]{figures/exported/01-factory-method-event-creator.png}}
\caption{图 3-1 工厂方法模式：统一创建不同来源的日程事件}
\end{figure}

这一模式的直接收益是：新增事件类型时只需要增加新的创建器，而不必破坏已有逻辑；同时单元测试可以针对单个创建器展开，降低后续维护成本。

\subsection{b. 单件（Singleton）}\label{b.-ux5355ux4ef6singleton}

单件模式要求某类在系统生命周期中只保留一个全局可访问实例。在 Python
与前端模块化开发中，单件模式经常不以传统 \texttt{getInstance()}
写法出现，而是通过模块级唯一对象体现。
他们的特点都是全局共享、重复创建没有意义、配置应保持一致。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-2 单件模式：全局共享配置与客户端实例}]{figures/exported/02-singleton-global-instances.png}}
...
\section{2. 结构型模式}\label{ux7ed3ux6784ux578bux6a21ux5f0f}

\subsection{a. 适配器（Adapter）}\label{a.-ux9002ux914dux5668adapter}

适配器模式用于把原本接口不兼容的对象转换成调用方期望的统一接口。在本项目中，最典型的适配器落点是\textbf{课表导入}。

当前我们的系统已经预留了 \texttt{POST\ /api/v1/courses/import} 和
\texttt{POST\ /api/v1/courses/ocr}
...
\texttt{CsvCourseAdapter}、\texttt{ExcelCourseAdapter} 和
\texttt{OcrCourseAdapter}，统一对外暴露
\texttt{parse(file):\ list\textless{}CourseDTO\textgreater{}}。上层导入服务不再关心原始输入的细节，而只处理标准化后的
\texttt{CourseDTO} 列表。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-3 适配器模式：多源课表导入适配为统一课程结构}]{figures/exported/03-adapter-course-import.png}}
\caption{图 3-3 适配器模式：多源课表导入适配为统一课程结构}
\end{figure}

采用适配器模式后，系统可以持续扩展新的导入来源，同时保证冲突检测、课程入库和日历展示只依赖统一的数据模型。

\subsection{b. 外观（Facade）}\label{b.-ux5916ux89c2facade}

外观模式通过提供更高层、更一致的访问入口，隐藏底层子系统的复杂度
{[}8{]}。在本项目中，这一模式已经清晰体现在前端 API 封装中。

当前前端不会在页面组件里直接使用 Axios，而是由 \texttt{src/api/http.js}
统一创建客户端实例，再由
\texttt{activities.js}、\texttt{recommendations.js}、\texttt{schedules.js}
等文件分别向页面暴露面向业务的请求函数。页面只需要调用
\texttt{getActivities()}、\texttt{getRecommendedActivities()} 或
\texttt{checkConflict()} 之类的接口，而不必关心
\texttt{baseURL}、\texttt{Authorization} 请求头和 \texttt{response.data}
解包等细节。
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-4 外观模式：前端 API 封装对页面提供统一访问入口}]{figures/exported/04-facade-frontend-api.png}}
\caption{图 3-4 外观模式：前端 API 封装对页面提供统一访问入口}
\end{figure}

\subsection{c.~代理（Proxy）}\label{c.-ux4ee3ux7406proxy}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-5 代理模式：管理员操作先经鉴权代理再进入真实业务}]{figures/exported/05-proxy-admin-auth.png}}
\caption{图 3-5 代理模式：管理员操作先经鉴权代理再进入真实业务}
\end{figure}

该模式能够明显降低权限逻辑和业务逻辑的耦合度，并为未来的管理员审计、统一错误提示和请求拦截器扩展留下空间。

\section{3. 行为型模式}\label{ux884cux4e3aux578bux6a21ux5f0f}

\subsection{a. 观察者（Observer）}\label{a.-ux89c2ux5bdfux8005observer}

观察者模式定义一种一对多依赖关系：当主题对象状态变化时，所有依赖它的观察者都会收到通知并更新。在前端响应式框架中，这种模式非常常见。

本项目当前最典型的观察者落点就是 \textbf{Pinia 登录态}。\texttt{auth}
store 维护 token
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-6 观察者模式：登录态变更驱动多个界面同步更新}]{figures/exported/06-observer-auth-store.png}}
...
\subsection{b. 策略（Strategy）}\label{b.-ux7b56ux7565strategy}
...
\texttt{InterestMatchStrategy}、\texttt{HotScoreStrategy}、\texttt{TimeProximityStrategy}、\texttt{CollegeRelatedStrategy}
与 \texttt{ConflictPenaltyStrategy}，再由推荐引擎统一组合和累加。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-7 策略模式：推荐引擎组合多种打分算法}]{figures/exported/07-strategy-recommendation-engine.png}}
\caption{图 3-7 策略模式：推荐引擎组合多种打分算法}
\end{figure}

采用策略模式后，本项目的推荐服务将具备更好的可解释性和可测试性：每种策略都可以单独验证，推荐理由也可以直接回溯到具体的得分来源。

\subsection{c.~状态（State）}\label{c.-ux72b6ux6001state}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-8 状态模式：课表导入任务在多个状态间有序流转}]{figures/exported/08-state-course-import.png}}
\caption{图 3-8 状态模式：课表导入任务在多个状态间有序流转}
\end{figure}

状态模式的优势在于：流程更清晰，错误恢复路径更明确，前端与后端也更容易围绕统一流程语义协作。

\subsection{d.~模板方法（Template
...
\texttt{AbstractImportPipeline}，固定流程顺序，再让
\texttt{CourseFileImportPipeline}、\texttt{OcrCourseImportPipeline} 和
\texttt{CrawlerImportPipeline} 分别实现特定步骤。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-9 模板方法模式：导入与采集流程复用统一处理骨架}]{figures/exported/09-template-method-import-pipeline.png}}
\caption{图 3-9 模板方法模式：导入与采集流程复用统一处理骨架}
\end{figure}

这样做能显著减少重复代码，也能保证不同导入路径遵循一致的错误处理和结果汇报机制。

\subsection{e. 中介者（Mediator）}\label{e.-ux4e2dux4ecbux8005mediator}
...
因此，更合理的做法是引入
\texttt{ActivityFilterMediator}，由它统一协调关键词输入、校区选择、类别选择和分页器的联动逻辑，并最终触发列表刷新。

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={图 3-10 中介者模式：筛选面板由统一协调者管理联动}]{figures/exported/10-mediator-filter-panel.png}}
...
\chapter{IV. 参考文献}\label{iv.-ux53c2ux8003ux6587ux732e}
