...
\begin{frame}{项目定位：从“发现活动”到“安排日程”的完整闭环}
...
\begin{frame}{结项版本功能：核心链路已经形成可运行闭环}
    ...
    \begin{tabular}{p{0.18\linewidth}p{0.56\linewidth}p{0.16\linewidth}}
        ...
        学术沉淀 & 个人中心、兴趣标签展示、活动行为记录、学术轨迹基础数据 & \good{基础闭环} \\
        \bottomrule
    \end{tabular}
...
\begin{frame}{总体架构：B/S、前后端分离与后端分层}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.58\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{1.3-系统层级结构.png}
        \end{column}
        \begin{column}{0.38\textwidth}
            \begin{itemize}
                \item \keyword{客户端表现层}：Vue 3、Element Plus、Pinia、Vue Router，负责页面与交互。
                \item \keyword{服务端接口层}：FastAPI 接收请求，完成参数校验、权限判断和统一响应。
                \item \keyword{业务逻辑层}：集中处理活动查询、推荐计算、课程导入和冲突检测。
                \item \keyword{数据访问层}：SQLAlchemy ORM 访问 MySQL，模型与 \texttt{schema.sql} 对齐。
                \item \keyword{外部数据源}：学院网站、课程表文件和活动截图。
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
